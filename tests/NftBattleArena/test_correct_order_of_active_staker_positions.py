...
def test_correct_order_after_voting(second_stage, accounts):
	((zooToken, daiToken, linkToken, nft), (vault, functions, governance, staking, voting, arena, listingList, xZoo, jackpotA, jackpotB)) = second_stage
	...
	tx1 = voting.createNewVotingPosition(3, amount1, {"from": accounts[0]})

	assert arena.numberOfNftsWithNonZeroVotes() == 1
	assert 3 == get_first_part_of_active_staker_positions(arena, 3)[0]
	
	tx2 = voting.createNewVotingPosition(1, amount2, {"from": accounts[1]})
